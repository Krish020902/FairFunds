...
const ContributionApp = () => {
  ...
  const [showRemainingNamesModal, setShowRemainingNamesModal] = useState(false);
  const [remainingNames, setRemainingNames] = useState([]);
  const [selectedRemainingName, setSelectedRemainingName] = useState("");
  ...
  const handleShowRemainingNames = (index) => {
    const remainingNames = names
      .filter((_, i) => i !== index)
      .map((person) => person.name);
    setRemainingNames(remainingNames);
    setShowRemainingNamesModal(true);
  };

  const handleSelectRemainingName = (name) => {
    setSelectedRemainingName(name);
  };

  const handleConfirmRemainingNames = () => {
    // Handle the selectedRemainingName as needed
    setShowRemainingNamesModal(false);
  };
  ...
  return (
    <View style={styles.container}>
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => (
              ...
              >
                ...
                <TouchableOpacity
                  onPress={() => handleShowRemainingNames(index)}
                  style={{
                    padding: 10,
                    marginLeft: 10,
                  }}
      ...
      <Modal
        visible={showRemainingNamesModal}
        animationType="slide"
        transparent={true}
        onRequestClose={() => setShowRemainingNamesModal(false)}
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <Text>Remaining Names</Text>
            <FlatList
              data={remainingNames}
              ...
              renderItem={({ item }) => (
                <View style={{ flexDirection: "row" }}>
                  <Text>{item}</Text>
                  <TextInput
                    style={styles.amountInput1}
                    keyboardType="numeric"
                  ></TextInput>
                </View>
              )}
            />
            <Button title="Confirm" onPress={handleConfirmRemainingNames} />
            <Button
              title="Cancel"
              onPress={() => setShowRemainingNamesModal(false)}
            />
          </View>
        </View>
      </Modal>
